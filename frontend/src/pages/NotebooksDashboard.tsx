...
const NotebookCard: React.FC<{ notebook: NotebookResponse }> = ({
                                                                  notebook,
                                                                }) => {
  return (
      <div
          className={`p-4 rounded-xl flex flex-col h-48 shadow-sm transition-shadow hover:shadow-md ${notebook.bg_color}`}
      >
        ...
        <div className="mt-auto">
          <h3
              className={`text-lg font-semibold ${notebook.text_color} leading-tight`}
          >
            {notebook.title}
          </h3>
          {/* Removed sourceCount as it's not in the new service response */}
          <p className="text-xs text-gray-500 mt-2">{notebook.date}</p>
        </div>
      </div>
  );
};

// Component for the "Create new notebook" card (no changes needed)
